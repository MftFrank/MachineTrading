...
%
% Change log:
%    2013-05-20: Fixes in preparation for HG2
%    2013-05-13: Fixed the panel's default backgroundcolor to be the same as their parent's bgcolor
...
% $Revision: 1.3 $  $Date: 2013/05/20 01:37:52 $
...
function [h1, h2, hDivider] = splitPanes(paramsStruct)
    ...
    for parentIdx = 1 : length(paramsStruct.parent)
        ...
        if ~isempty(c),  try hDivider(parentIdx) = c; catch, hDivider = c; end,  end
    end
...
function listeners = addPropListeners(hFig, hDivider, h1, h2, propNames)
    ...
    try
        ...
        %listeners(end+1) = handle.listener(hhDivider, findprop(hhDivider,'Position'), 'PropertyPostSet', @dividerResizedCallback);
    end
